...
export function AgentsRail({ onNew, onSelect, mobile }: { onNew: () => void; onSelect?: () => void; mobile?: boolean }) {
  ...
  return (
    <div style={{ width: mobile ? '100%' : 280, flex: mobile ? 1 : undefined, flexShrink: 0, background: 'var(--color-pane)', borderRight: mobile ? 'none' : '1px solid var(--color-border)', overflow: 'auto', padding: 8 }}>
      ...
      {filtered.map((s) => (
        <button key={s.id} onClick={() => { void useAgents.getState().select(s.id); onSelect?.(); }} style={{
