...
export function NewClaudeThreadModal({ sessionId, onClose, onCreated }: { sessionId: string; onClose: () => void; onCreated: (id: string) => void }) {
  ...
  async function create(externalId?: string) {
    ...
    setBusy(true);
    try {
      const t = await api.createTerminal(sessionId, { type: 'claude-code', label: name.trim() || undefined, externalId });
  ...
  return (
    <Modal open onClose={onClose} title="New Claude Code Thread">
